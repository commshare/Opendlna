									  ...
									  ProgressCallBack header_processcb)
{
	...
	CMultiRequest hotvrs_client(Response_Whole_DataType);
	//使用手动重置为无信号状态，初始化时无信号状态
	bresult = hotvrs_client.AddRequest(url, 0, HotVrsResponseCallBackFunc, ProgressCB,NULL,this);
	if (!bresult)
		return -1;
	...
	for (size_t i=0; i < section_count; i++)
	{
		...
		bresult = cdn_client.AddRequest(W2A(wstream.c_str()), 0, CdnResponseCallBackFunc, ProgressCB,NULL,this);
		if (!bresult)
			return -1;
	}
	...
	for (size_t idx = 0; idx < section_count; idx++)
	{
		...
		USES_CONVERSION;
		bresult = header_client.AddRequest(W2A(wstream.c_str()), 0, HeaderResponseCallBackFunc, ProgressCB,NULL,this);
		if (!bresult)
			return -1;
	}
